    ...
    insert_easyonamejs_applet: function(Y, toreplaceid, appletid, name,
        ...
        if (!this.show_java(toreplaceid, appletid, name, 600, 460,
		 ...
		 //console.log(e.target.get('href'));
				...
				Y.use("node", "io", 'json-parse', 'json-stringify', 'querystring-stringify-simple', function(Y) {
                                    var mdlmoldata = source.split("\n").join("\r\n");
                                    mdlmoldata = encodeURIComponent(mdlmoldata);
				    var uri = M.cfg.wwwroot + "/question/type/easyonamejs/openbabel.php?mdlmol=" + mdlmoldata + "&xformat=mol";
				    // Define a function to handle the response data.
				    function complete(id, o) {
					var id = id; // Transaction ID.
					var data = o.responseText; // Response data.

				        //Y.one('#' + textfieldid).set('value', data);
                                        console.log(data);
                                        console.log(topnode);
                                        //return data;
				        Y.one(topnode + ' input.answer').set('value', data);
					inputdiv.ancestor('form').submit();
                                        
				    };
				    Y.on('io:complete', complete, Y);
				    // Make an HTTP request to uri.
...
/*
	$.ajax({
		type: 'POST',
		url: 'openbabel.php',
		data: {mdlmol: $('#mdlmol').val(), xformat: format},
		dataType: 'json',
		success: function (data) {
		//console.log(data);
		$('#molsource-box').val(data);
		}
	});
*/
            ...
            MarvinJSUtil.getEditor("#MSketch").then(
			    ...
			    Y.use("node", "io", 'json-parse', 'json-stringify', 'querystring-stringify-simple', function(Y) {
				    ...
				    var request = Y.io(uri);
         	        ...
         	        $.ajax({
			    type: 'POST',
			    url: 'type/easyonamejs/openbabel.php',
			    data: {mdlmol: source, xformat: 'can'},
			    dataType: 'json',
			    success: function (data) {
			    //console.log(data);
                            alert(data);
